#= A version of "resonant" that uses L-BFGS instead of BFGS.

This is a "hack" script, not wholly robust - it won't be able to perfectly restart.

Properly, the state for L-BFGS should maintain a list of the last however many gradients,
    or something like that.
Really it should get its own job directory.
But that's too much work, so we're just going to hack away ALL the gradient information.
...
#= HACK: Forget Hk and never try to remember it again... =#
state.Hk = Matrix{eltype(state.Hk)}(undef, 0, 0)

import Optim, LineSearches, Printf

# SELECT THE OPTIMIZATION METHOD
linesearch = LineSearches.MoreThuente()
bfgs_optimizer = Optim.LBFGS(linesearch=linesearch) #= HACK: Change to BFGS. =#
...
# bfgs_state.invH .= _!.state.Hk    #= HACK: Comment out this line. =#
...
function bfgs_callback(callback_state)
    ...
    # _!.state.Hk .= bfgs_state.invH    #= HACK: Comment out this line. =#
    JOB.save()
    ...
    if iteration[] % _!.meta.update == 0
        ...
        # JOB.archive(JOB.iterid(iteration[]))  #= HACK: Comment out this line. =#
    end
...
# JOB.archive("final")    #= HACK: Comment out this line. =#
