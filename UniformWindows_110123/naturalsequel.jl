#= This file is supposed to be run only when a `natural` job has been loaded.. =#
...
ifun = construct__staticevolver(evolution, protodevice, basis, grid, ψ0)
@time ijac = construct__jacobian(ifun, protodevice)
iUSV = LinearAlgebra.svd(ijac)
ipulsesets = construct__naturalpulsesets(protodevice, ijac)

# All that is how we made the first device. Now bind the current parameters, to extract amplitudes.
interdevice = CtrlVQE.Systematic(NormalTransmonDevice, system.n, ipulsesets; m=setup.m)
CtrlVQE.Parameters.bind(interdevice, _!.state.x)
for i in 1:CtrlVQE.ndrives(protodevice)
    intersignal = CtrlVQE.drivesignal(interdevice, i)
    signal = CtrlVQE.drivesignal(protodevice, i)
    # TODO: obv. none of this is general. This is a total hack. This is a pain. >_>
    for k in eachindex(signal.starttimes)
        Omega = CtrlVQE.valueat(intersignal, signal.starttimes[k])
        signal.windows[k].A = real(Omega)
        signal.windows[k].B = imag(Omega)
    end
end

# now reset. Very likely the degenerate pairs have mixed some, so the parameters from before don't necessarily transfer.
_!.state = nothing
_!.trace = nothing
_!.outdir *= "_sequel"
