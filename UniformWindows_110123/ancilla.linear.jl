...
Anyway, we'll try the 2xN architecture in the `ancilla` script,
    and in this one we'll use a "control" where the ancilla are just linearly appended to the system register.
...
device = CtrlVQE.Systematic(
    devicetype,
    2*system.n,                         # NOTE: Use twice as many qubits.
    CtrlVQE.UniformWindowed(protopulse, setup.T, setup.W),
    m = setup.m,
)
# append!(device.quples, CtrlVQE.Quple(q,1+2*system.n - q) for q in 1:system.n)
# append!(device.ḡ, device.ḡ[1:system.n])
#     # Zips the systematic linear coupling into a ladder instead of a line.
    # The ONLY difference between `ancilla` and `ancilla.linear` is the extra couplings.

ψ0 = CtrlVQE.QubitOperators.project(CtrlVQE.kron(       # Extend statevector on ancillae.
    system.ψ0,
    CtrlVQE.LinearAlgebraTools.basisvector(length(system.ψ0), 1),
), device)
