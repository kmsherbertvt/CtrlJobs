...
module Printing
    ...
    inspect(vars::Vars) = inspect(vars, :system, :adaptation, :optimization, :windows)
    ...
    function __setup__(setup, flags)
        ...
        :convergence in flags && (notepad *= __fields__(setup,
            "Convergence Variables",
            (:f_tol, :g_tol, :maxiter, :fnRATIO),
        ))
        return notepad
    end
    ...
    function __trace__(trace, flags; maxshow=5)
        ...
        nshowa = min(maxshow, nadapt)
        nrecords = length(trace.iterations)
        nshow = min(maxshow, nrecords)

        notepad = ""
        :adaptation   in flags && (notepad *= __trace__adaptation(trace, nadapt, nshowa))
    ...
    function __trace__adaptation(trace, nrecords, nshow)
        template = Printf.Format("%6s    "^3 * "%22.22s    "^2 * "\n")

        notepad = "\n\n***   Adapt Trace   ***\n\n"
        notepad *= __trace__header(
            template, nrecords, nshow,
            ("Adapt", "Iter", "# f", "E", "G_max"),
        )
        for i in 1:nshow
            i += nrecords - nshow
            a = trace.adaptations[i]
    ...
    function __state__(state, flags)
        ...
        notepad = ""
        :windows in flags && (notepad *= __state__modes(state))
    ...
    function __state__modes(state)
        ...
        -----
        """
        for i in eachindex(state.n)
            notepad *= Printf.format(template, "s[$i]", state.n[i])
        end
        return notepad
    end
    ...
    function __state__counts(state)
        template = Printf.Format("%8s: %s\n")
        notepad = "\n\n*** Parameter Counts ***\n\n"
        for i in eachindex(state.Ω)
            notepad *= Printf.format(template, "# Ω[$i]", length(state.Ω[i]))
        end
        notepad *= Printf.format(template, "# ν", length(state.ν))
        notepad *= Printf.format(template, "Total", length(state.x))
        return notepad
    end
    ...
    function __state__arrays(state)
        ...
        """
        for i in eachindex(state.Ω)
            notepad *= Printf.format(template, "Ω[$i]", state.x[state.Ω[i]])
        end
    ...
    function report(vars::Vars, iter::Int, flags::Symbol...)
        ...
        a = findlast(i -> i <= iter, vars.trace.adaptations)
        adapt = isnothing(a) ? 0 : a

        notepad = """

        Reporting on iteration #$iter (adapt #$adapt)
        ---------------------------------------------
        """
