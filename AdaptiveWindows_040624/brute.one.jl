#= Just a brute-force script to double-check that nodes.one is optimal as I claim.

Results from 24ns:
    First 14 adaptations are identical except for off-by-one errors in the time grid,
        as I expected. If we implemented the brute pool using interpolation,
        I expect they would indeed be identical.
    Somewhat more surprising is that in adaptation 15, these off-by-one errors
        are evidently sufficient for optimization to have converged
        to somewhat different values in one of the pulses,
        resulting in a somewhat different gradient signal,
        which resulted in a different trajectory from then on.
    The "brute-force" trajectory wound up being a little better,
        but I think this is accidental so I'm not going to sweat it.
