...
- Candidates are randomly selected from legal times, for each drive.
- Split every existing window into two (if allowed by bandwidth constraint).
...
function do_optimization()
    ...
    println("*"^40)
    println(size(bfgs_state.x))
    println(size(bfgs_state.invH))
    println(size(_!.state.x))
    println(size(_!.state.Hk))
    println("*"^40)
